...
async function readJson(req) {
  if (req.body !== undefined && req.body !== null) {
    if (typeof req.body === "string") {
      if (Buffer.byteLength(req.body, "utf8") > 1_000_000) throw new Error("Request is too large");
      return JSON.parse(req.body || "{}");
    }
    if (Buffer.isBuffer(req.body)) {
      if (req.body.length > 1_000_000) throw new Error("Request is too large");
...
// A serverless instance can disappear between clicks, so the browser carries the receipt forward.
...
async function handleApiRequest(req, res, requestUrl) {
  ...
  const pathname = action ? `/api/${action}` : url.pathname;
